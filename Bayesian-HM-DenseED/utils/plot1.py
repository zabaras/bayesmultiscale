...
def plot_prediction_det1(target, mean_predict, std_predict, epoch, index, 
                        plot_fn='contourf'):
    ...
    """
    samples = np.concatenate((target,mean_predict,np.abs(target-mean_predict), std_predict), axis=0)
    ...
    plt.savefig('./result_plot/Var_KLE_{}_sample_{}.png'.format(index,epoch),dpi=300, bbox_inches='tight')
    plt.close(fig)
