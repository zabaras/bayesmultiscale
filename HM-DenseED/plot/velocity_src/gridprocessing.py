...
def computeGeometry(G, findNeighbors=False, hingenodes=None):
    ...
    if G.gridDim == 3:
        ...
        faceNumbers = rldecode(np.arange(G.faces.num), np.diff(G.faces.nodePos,axis=0))
        ...
        cellNumbers = rldecode(np.arange(G.cells.num), numFaces)
...
def _findNeighbors(G):
    ...
    """
    # Internal faces
    cellNumbers = rldecode(np.arange(0, G.cells.num), np.diff(G.cells.facePos, axis=0))
...
def _averageCoordinates(n, c):
    no = rldecode(np.arange(n.size), n)
